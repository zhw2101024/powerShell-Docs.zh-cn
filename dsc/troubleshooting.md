...
* **Newest**：采用整数值指示要显示的操作数。 默认情况下，它将返回 10 个最新的操作。 例如，TODO：显示 Get-xDscOperation -Newest 5
...
例如，下面的输出显示多台计算机中最后一项操作的结果：   TODO：替换 Get-xDscOperation 的图片以显示远程计算机日志
...
这将显示与 `Get-WinEvent` cmdlet 相同的结果，如以下输出中所示：   TODO：输出内容是什么？
