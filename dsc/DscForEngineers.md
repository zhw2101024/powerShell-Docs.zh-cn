...
“连续部署”的概念变得更加重要。 连续部署是指能够频繁部署，可能每天进行多次部署。
这些部署的目的不是为了提供修复，而是为了进行快速发布。
通过尽可能顺畅可靠地完成新功能从开发到操作的过程，可以缩短新的业务逻辑转换为价值的时间。

以下两种正在作用的趋势加大了快速转移的这种需求。 向云计算转移意味着出现规模的快速变化，同时经常出现失败威胁。
这些影响迫使进行自动化，以便跟上潮流。
创建“DevOps”理念就是为了应对这些变化。 


DSC 是一个提供声明式、自治式和幂等式（可重复）部署、配置和符合项的平台。
...
```
虽然此脚本简单易懂。 但是，如果尝试将该脚本投入生产环境，将会遇到一些问题。
如果该脚本连续运行两次会发生什么情况？
