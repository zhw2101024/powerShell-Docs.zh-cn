...
所需状态配置 (DSC) 是一个管理平台，借助它可以通过使用名为托管对象格式 (MOF) 的行业语法描述通用信息模型 (CIM)，来部署和管理配置数据。 开放式管理基础结构 (OMI) 这一开放源代码项目的存在是为了跨平台（包括 Linux 和网络硬件操作系统）进一步开发这些标准。 有关详细信息，请参阅[链接到 MOF 规范的 DMTF 页面](http://dmtf.org/standards/cim)和 [OMI 文档和源](https://collaboration.opengroup.org/omi/documents.php)。
...
使用来自 **PowerShellGet** 模块的 **Install-Module** cmdlet。
...
为 DNS 记录选择名称时，请记住解决方案体系结构。 如果使用负载平衡，则用于在 HTTPS 上保护流量安全的证书需要共享与 DNS 记录相同的名称。 同样，如果使用高可用文件共享，则会使用群集的虚拟名称。

方案 |最佳做法
:---|:---
测试环境 |在可能时重现计划生产环境。 服务器主机名适用于简单配置。 如果 DNS 不可用，则可以使用 IP 地址替代主机名。|
单节点部署 |创建指向服务器主机名的 DNS CNAME 记录。|
高可用部署 |如果客户端通过负载平衡解决方案进行连接，则创建用于虚拟 IP 的主机名以及引用该主机名的 CNAME 记录。 如果使用 DNS 轮循机制在请求服务器间分发客户端请求，则必须配置名称记录以包括所有已部署的请求服务器实例的主机名。|

有关详细信息，请参阅[在 Windows Server 中配置 DNS 轮循机制](https://technet.microsoft.com/en-us/library/cc787484(v=ws.10).aspx)。
...
#### <a name="designing-for-high-availability"></a>为实现高可用性而设计  
请求服务器角色可以部署在高可用体系结构中。 Web 服务角色可以进行负载平衡，包含 DSC 模块和 DSC 配置的文件和文件夹可以位于高可用存储上。

请记住，配置和模块传递给目标节点之后，执行测试和设置配置所需的所有数据都会以本地方式存储在每个节点上。 从请求服务器只传递更改。 除非部署处于活动状态，否则请求服务器的服务故障不会成为中断。  通常，只针对最大环境保证高可用性。

配置高可用请求服务器环境需要有关如何在多个服务器节点间分发客户端请求，以及如何在这些节点间共享所需服务器文件的决策。
...
在多台服务器配置为请求服务器并且在它们间对连接进行平衡负载的高可用方案中，从这些服务器提供的资源和配置应是相同的，这一点至关重要。 实现此目标的最佳方式是将此内容存储在高可用位置上（如群集文件共享）。 可以在每台服务器的配置中指定共享的位置。有关共享存储选项的详细信息，请参阅“应用程序数据的横向扩展文件服务器概述”。
...
GUID 是应视为敏感数据的信息，因为它可以由具有恶意企图的人员用于获取有关如何在环境中部署和配置服务器的情报。 有关详细信息，请参阅[在 PowerShell 所需状态配置请求模式中安全地分配 GUID](http://blogs.msdn.com/b/powershell/archive/2014/12/31/securely-allocating-guids-in-powershell-desired-state-configuration-pull-mode.aspx)。
