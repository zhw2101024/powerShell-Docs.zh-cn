...
如果需要，cmdlet 必须定义用于处理输入的参数。 此获取处理器 cmdlet 定义一个**Name**参数[，如添加处理命令行输入的参数](adding-parameters-that-process-command-line-input.md)中所述。
...
向 PowerShell 注册 cmdlet 后，可以通过在命令行上运行 cmdlet 来对其进行测试。 让我们测试该示例的 Get Proc cmdlet，以查看它是否报告错误：
