...
使用 Windows PowerShell Web 服务需要第三方来实现[CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)接口，以公开 Windows PowerShell cmdlet。 此接口对 web 服务执行用户授权。 编写代码来实现接口后，必须将其编译到要在 web 应用程序中使用的 DLL。
