---
ms.date: 2017-06-05T00:00:00.000Z
keywords: powershell,cmdlet
title: "基本概念"
ms.openlocfilehash: d7b009780636b6938492869d70d53d4d615fbf79
ms.sourcegitcommit: 74255f0b5f386a072458af058a15240140acb294
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/03/2017
---
#  <a name="fundamental-concepts"></a>基本概念

-  [关于 Windows PowerShell](fundamental/About-Windows-PowerShell.md)
-  [探究 Windows PowerShell ISE](fundamental/Exploring-the-Windows-PowerShell-ISE.md)
-  [获取详细的帮助信息](fundamental/Getting-Detailed-Help-Information.md)
-  [获取有关命令的信息](fundamental/Getting-Information-About-Commands.md)
-  [学习 Windows PowerShell 名称](fundamental/Learning-Windows-PowerShell-Names.md)
-  [对象管道](fundamental/Object-Pipeline.md)
-  [使用 Windows PowerShell 编写脚本](fundamental/Scripting-with-Windows-PowerShell.md)
-  [了解重要的 Windows PowerShell 概念](fundamental/Understanding-Important-Windows-PowerShell-Concepts.md)
-  [了解 Windows PowerShell 管道](fundamental/Understanding-the-Windows-PowerShell-Pipeline.md)
-  [使用熟悉的命令名称](fundamental/Using-Familiar-Command-Names.md)
-  [使用变量存储对象](fundamental/Using-Variables-to-Store-Objects.md)
-  [使用 Windows PowerShell 进行管理](fundamental/Using-Windows-PowerShell-for-Administration.md)
-  [使用 Windows PowerShell](fundamental/Using-Windows-PowerShell.md)
-  [Windows PowerShell 基础知识](fundamental/Windows-PowerShell-Basics.md)
-  [Windows PowerShell 集成脚本环境 (ISE)](fundamental/Windows-PowerShell-Integrated-Scripting-Environment--ISE-.md)

