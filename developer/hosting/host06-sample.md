...
此示例演示如何生成基于控制台的交互式主机应用程序，从命令行读取并执行命令，然后将结果显示到控制台。 此外，此示例还使用了 Tokenizer API 来指定用户输入的文本颜色。
...
## <a name="example"></a>示例
 ...
 下面的代码读取命令行和颜色的文本输入。 使用确定令牌[System.Management.Automation.Psparser.Tokenize*](/dotnet/api/System.Management.Automation.PSParser.Tokenize)方法。
