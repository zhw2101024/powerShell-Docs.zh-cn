...
你已经完成全部创建管理 OData web 服务所需的步骤后，必须将其部署为在 IIS 中的 web 应用程序。

## <a name="deploying-the-web-service"></a>部署 web 服务

完成以下步骤来部署管理 OData web 服务。

1. 为在 IIS 下 web 应用程序创建一个目录`WWWRoot`目录。

2. 复制 MOF 架构文件、 XML 架构文件导出的 Dll [Microsoft.Management.Odata.Customauthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)并[System.Management.Automation.Remoting.Pssessionconfiguration](/dotnet/api/System.Management.Automation.Remoting.PSSessionConfiguration)接口和到应用程序目录的 web.config 文件。
...
以下 Windows PowerShell 脚本演示如何部署管理 OData web 服务。
