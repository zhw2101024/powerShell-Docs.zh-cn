...
创建计划的作业的第一步指定作业的运行。 通过创建和配置 T:Microsoft.PowerShell.ScheduledJob.ScheduledJobTriggerobject 执行此操作。 以下代码创建计划来运行一次在将来 20 秒的作业触发器。
...
然后，创建 ScheduledJobInvicationInfo 和 SheduledJobDefinition 运行作业的对象。 下面的代码演示此操作。
