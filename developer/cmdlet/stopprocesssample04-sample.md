...
此示例演示如何编写声明参数集，指定默认参数集，它可以接受输入的对象的 cmdlet。 此 cmdlet 类似于是`Stop-Process`cmdlet 提供的 Windows PowerShell 2.0。

### <a name="how-to-build-the-sample-by-using-visual-studio"></a>如何通过使用 Visual Studio 生成该示例。

1. 安装了 Windows PowerShell 2.0 sdk，导航到 StopProcessSample04 文件夹。 默认位置为 C:\Program Files (x86) \Microsoft SDKs\Windows\v7.0\Samples\sysmgmt\WindowsPowerShell\csharp\StopProcessSample04。
...
下面的代码演示声明参数集，指定默认参数集，它可以接受输入的对象的 Stop Proc cmdlet 实现。

此示例演示输入的对象、 如何声明参数集，以及如何指定默认参数设置为使用。
