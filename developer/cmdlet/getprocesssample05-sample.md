...
此示例演示 Get-proc cmdlet 的完整版本。

## <a name="how-to-build-the-sample-using-visual-studio"></a>如何使用 Visual Studio 生成示例。

1. 打开 Windows 资源管理器并导航到示例目录下的 GetProcessSample05 目录。

   安装了 Windows PowerShell 2.0 sdk，导航到 GetProcessSample05 文件夹。 默认位置为 C:\Program Files (x86) \Microsoft SDKs\Windows\v7.0\Samples\sysmgmt\WindowsPowerShell\csharp\GetProcessSample05。

2. 双击解决方案 (.sln) 文件的图标。 这将在 Visual Studio 中打开示例项目。

3. 在中**构建**菜单中，选择**生成解决方案**。

   将默认的 \bin 或 \bin\debug 目录中生成的示例库。
...
- 指定的参数可以采用输入管道中。 输入可以来自对象或从一个对象，其属性名称为参数名称相同的属性值。
