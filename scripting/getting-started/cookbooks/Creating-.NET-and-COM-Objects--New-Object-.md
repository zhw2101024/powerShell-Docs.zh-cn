...
在某些情况下，COM 对象可能具有相关联的 .NET Framework *运行时可调用包装器*（或称 RCW），它将由 **New\-Object** 使用。 因为 RCW 的行为可能与普通 COM 对象的行为不同，所以 **New\-Object** 具有 **Strict** 参数，它将对 RCW 访问进行警告。 如果指定 **Strict** 参数然后创建使用 RCW 的 COM 对象，你会收到一条警告消息：
