...
> [!NOTE]
> 此脚本创建的列表框只允许进行单选。 若要创建允许进行多选的列表框控件，请指定 **SelectionMode** 属性的值，类似于以下内容：`$listBox.SelectionMode = "MultiExtended"`。 有关详细信息，请参阅[多选列表框](../Topic/Multiple-selection-List-Boxes.md)。
