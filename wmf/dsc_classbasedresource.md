...
运行此配置，就像对任何 DSC 配置脚本进行的操作一样。 若要在已提升权限的 Windows PowerShell 控制台中启动该配置，运行以下 cmdlet。 
你会发现 FileResource 中的 Get-DscConfiguration 输出包含的信息比 BaseFileResource 多。
